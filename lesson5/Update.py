...
def hashtable_update(htable,key,value):
    ...
    test_key = False        # remove
    for entry in bucket:
        if(entry[0] == key):
            entry[1] = value
            test_key = True # another way is to put return and remove test_key
    if(test_key == False):  # remove
       bucket.append([key,value]) 
    
    return htable

#####################################################################
# Answer is above and I answered it with no help like most this lesson
###########################################################################
...
print table
#>>>[[['Francis', 13], ['Ellis', 11]], [['Andy', 5], ['Luke', 7]],[['Bill', 17], ['Zoe', 14]], 

...
# Stars! #
product = "lunch"
debris = "loops and punch"
#print debris.find(product)
def fix_machine(debris, product):
    # get size of product string, loop the charcters of product and compare to debris,
    # compare each letter of product to debris, if only 1 can't be found (return message)
    # if all characters can be found in debris from product -> return product if not -> return message
    size = len(product)
    s2 = len(debris)
    #str holdword 
    i = 0
...
print fix_machine('UdaciousUdacitee', 'Udaciiiyte')
### TEST CASES ###

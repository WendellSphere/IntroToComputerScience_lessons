...
def record_user_click(index,keyword,url): #this was added to run it
    urls = lookup(index,keyword)
    if urls:
        for entry in urls: # for entry in [[[url, 0]]
            if(entry[0] == url):
                entry[1] = entry[1] + 1

def add_to_index(index, keyword, url): #this was modfied to change datastruct
    for entry in index:
        if entry[0] == keyword:
